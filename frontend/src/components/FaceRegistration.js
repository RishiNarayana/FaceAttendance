...
const FaceRegistration = ({ onComplete }) => {
  ...
  const startRegistrationFlow = async () => {
    ...
    if (!videoRef.current || !videoRef.current.srcObject) return alert("Please start the camera first.");
    ...
    // Capture a frame every 400ms until we have 5 solid embeddings
  ...
  const finishRegistration = async () => {
    ...
    // Average all 5 128-d embeddings to create 1 highly robust master embedding
    const masterEmbedding = new Array(128).fill(0);
    embeddingsRef.current.forEach(emb => {
      for (let i = 0; i < 128; i++) {
        masterEmbedding[i] += emb[i];
      }
    });
    for (let i = 0; i < 128; i++) {
        masterEmbedding[i] = masterEmbedding[i] / 5;
    }
    ...
    try {
      ...
      alert("Failed to register face: " + (err.response?.data?.msg || err.message));
  ...
  return (
    <div style={{ padding: "20px", border: "1px solid #E5E7EB", borderRadius: "10px", marginTop: "15px", backgroundColor: "white", boxShadow: "0 4px 6px -1px rgba(0,0,0,0.05)" }}>
      ...
      <div style={{ position: "relative", width: "100%", maxWidth: "320px", margin: "0 auto 20px auto" }}>
        ...
        {isCapturing && (
          <div style={{
            ...
            borderRadius: "8px", border: `4px solid ${progress === 100 ? "#10B981" : "var(--secondary)"}`,
            ...
            <div style={{ 
              ...
              <div style={{ width: `${progress}%`, height: "6px", backgroundColor: "#10B981", borderRadius: "5px", transition: "width 0.4s ease" }}></div>
            </div>
            <div style={{
      ...
      {!isCapturing && (
        <div style={{ display: "flex", gap: "10px", justifyContent: "center" }}>
          <button 
            onClick={startCamera} 
            style={{ padding: "10px 15px", cursor: "pointer", backgroundColor: "white", color: "#374151", border: "1px solid #D1D5DB", borderRadius: "5px", fontWeight: "600", flex: 1 }} 
          ...
          <button 
            ...
            style={{ padding: "10px 15px", cursor: "pointer", backgroundColor: "var(--secondary, #8B5CF6)", color: "white", border: "none", borderRadius: "5px", fontWeight: "600", flex: 1 }} 
            disabled={!modelsLoaded}
          >
            2. Begin Scan
          </button>
